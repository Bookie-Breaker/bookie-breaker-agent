...
stores the Odds API event id. No shared key exists, but lines-service
...
are cached in Redis for 24h (``agent:gamemap:{stats_game_uuid}``).
...
class GameReconciler:
    ...
    async def resolve(self, game: Game) -> str | None:
        ...
        cache_key = f"{CACHE_PREFIX}{game.id}"
        try:
        ...
        for snapshot in snapshots:
            ...
            if snapshot.side == "HOME" and selection.startswith(home_name):
                await self._set_cached(cache_key, snapshot.game_id)
    ...
    async def _set_cached(self, cache_key: str, external_id: str) -> None:
        try:
            await self._redis.set(cache_key, external_id, ex=self._ttl)
        except Exception:  # noqa: BLE001 - cache is best-effort
            logger.warning("failed to cache game mapping %s", cache_key, exc_info=True)
